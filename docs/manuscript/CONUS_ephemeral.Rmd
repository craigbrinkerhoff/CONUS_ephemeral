...
#validation results
tar_load(val_shapefile_fin) #main results shapefile for mapping
val_shapefile_fin <- val_shapefile_fin$shapefile
tar_load(flowingDaysValidation) #main results for number ephemeral flowing days analysis (fig 3)
tar_load(combined_percEph_firstOrder) #results for first order (for some summary stats)
tar_load(validationPlotMain) #Q validation df (to get n validation sites)
tar_load(validationResults) #ephemeral classification validation (to get n validation sites)
tar_load(validated_wells) #groundwater vlaidation wells and gauges (to get n validation sites)
...
east <- c('0101', '0102', '0103', '0104', '0105', '0106', '0107', '0108', '0109', '0110', #basins determined to be east of the Mississippi river
          ...
          '0901', '0902', '0903', '0904')
west <- combined_results[!(combined_results$huc4 %in% c(east)),]$huc4 #all other basins are determined to be west of the Mississippi River

#manually specific HUC2 regions for paper summary stats
...
############SUMMARISE RESULTS-----------------------------
#stats for Q
relativeExport <- summary(combined_results$percQEph_exported) #by basin
relativeLength <- summary(combined_results$perc_length_eph) #by basin
relativeExport_order1 <- summary(combined_results_by_order[combined_results_by_order$StreamOrde==1,]$percQEph_reach_median) #first-order results- these are the within-basin medians for first order reaches (see ms)
relativeExportEast_order1 <- summary(combined_results_by_order[combined_results_by_order$StreamOrde==1 & combined_results_by_order$huc4 %in% east,]$percQEph_reach_median) #first-order, east- these are the within-basin medians for first order reaches (see ms)
relativeExportWest_order1 <- summary(combined_results_by_order[combined_results_by_order$StreamOrde==1 & combined_results_by_order$huc4 %in% west,]$percQEph_reach_median) #first-order, west- these are the within-basin medians for first order reaches (see ms)

#stats for DA
relativeExport_da <-  summary(combined_results$percAreaEph_exported) #by basin

#stats by stream order
...
#Overall validation numbers and results
...
perc_riverSource_eph <- round((sum(combined_percEph_firstOrder$lenKM_eph_source, na.rm=T) / sum(combined_percEph_firstOrder$lenKM_source, na.rm=T))*100,0) #%ephemeral for source reaches with no upstream reaches (the terminal subset of first-order)
perc_firstOrder_eph <- round((sum(combined_percEph_firstOrder$lenKM_eph_firstOrder, na.rm=T) / sum(combined_percEph_firstOrder$lenKM_firstOrder, na.rm=T))*100,0) #%ephemeral for all first-order reaches
...
All data sources are available in the manuscript or the supplementary materials. Code used to run our analysis and build our figures is available at https://github.com/craigbrinkerhoff/CONUS_ephemeral and archived at [@brinkerhoffCraigbrinkerhoffCONUS_ephemeralV12024a]. Model results are archived at [@brinkerhoffEphemeralStreamWater2024]. Note that continental-scale models prioritize capturing broad patterns across large domains, so we caution against over-interpreting results for individual river reaches.
...
df <- df[,c(2,5,6,7,8)] #have to grab manually, unfortunately
knitr::kable(df, format = "pipe", col.names = c('Drainage Basin', 'Observed Nflw [dys]', 'Number Sample Years', 'Number Sites','Reference'), row.names = FALSE)
```
