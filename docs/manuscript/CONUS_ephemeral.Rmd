...
In this context, we use a simple geomorphic model to identify ephemeral stream contributions to drainage networks. First, we compare modeled long-term monthly water table depths [@fanGlobalPatternsGroundwater2013; @fanHydrologicRegulationPlant2017] with streamline presence in `r formatC(nreaches, format="d", big.mark=",")` discrete streams, rivers, lakes, reservoirs, canals, and ditches across the contiguous United States (CONUS) [@u.s.geologicalsurveyNationalHydrographyDataset2022]. Using this analysis and a published river-lake-reservoir routing framework [@brinkerhoffLakeMorphometryRiver2021], we identify ephemeral streams as those with no groundwater connection across all twelve months (see materials and methods). Then, in conjunction with a published mean annual flow model [@u.s.geologicalsurveyNationalHydrographyDataset2022], we quantify the fraction of every river's discharge that was contributed by upstream CONUS ephemeral streams. Finally, we use 27 years of daily interpolated precipitation data [@xieGaugeBasedAnalysisDaily2007], as well as runoff and in situ sensor data (tables S`r table_nums("data_summary", display="n")`-S`r table_nums("Nflw_data", display="n")`), to quantify how frequently ephemeral streams flow in each basin (see materials and methods). We define and discuss the metrics that make up our assessment (equations S1-S3, table S`r table_nums("classVal", display="n")`), validate individual model components at up to `r formatC(nMapValidation, format="d", big.mark=",")` in situ sites across CONUS (supplementary text S1, figs. S`r fig_nums_supp("mainVal", display="n")`-S`r fig_nums_supp("walnutGulch", display="n")`, table S`r table_nums("Nflw_data", display="n")`, table S`r table_nums("field_data", display="n")`), and explore parameter settings and sensitivity (figs. S`r fig_nums_supp("classificationSens", display="n")`-S`r fig_nums_supp("runoffCalib", display="n")`). By leveraging a parsimonious modeling approach and validating our analysis at thousands of in situ sites, our goal is not simply to show that ephemeral streams occur, but to elucidate how their prevalence shapes downstream hydrology. 

We conclude by relating our scientific findings to ongoing debate surrounding the jurisdictional scope of the United States Clean Water Act (CWA) and whether it applies to ephemeral streams. The CWA grants federal agencies the authority to regulate the 'Waters of the United States' [WOTUS- @CleanWaterAct1985], and while there is general agreement that WOTUS includes large interstate waterways characterized as 'navigable waters', differing and conflicting interpretations apply to smaller/tributary interconnected waterways, especially those that are ephemeral [@walshOverviewEvolvingJurisdictional2022; @keiserConsequencesCleanWater2019]. At present, no majority United States Supreme Court opinion defines the scope of WOTUS, leaving regulators to navigate among competing definitions such as waters that are 'inseparably bound up' with navigable waters [@UnitedStatesRiverside1985], waters that hold a 'significant nexus' with navigable waters [@RapanosUS2006], and- most narrowly- waters that are 'relatively permanent, standing or continuously flowing bodies of water' [@RapanosUS2006]. In the past decade alone, federal agencies under three administrations have all promulgated rules with differing definitions of the CWA's jurisdictional scope [@u.s.armycorpsofengineersdepartmentofthearmydepartmentofdefenseCleanWaterRule2015; @u.s.armycorpsofengineersdepartmentofthearmydepartmentofdefenseNavigableWatersProtection2020; @u.s.armycorpsofengineersdepartmentofthearmydepartmentofdefenseRevisedDefinitionWaters2023]. This issue may be addressed once again in a case currently before the United States Supreme Court [@SackettEnvironmentalProtection2022]. Our assessment provides a comprehensive appraisal of ephemeral streams across CONUS and may help in establishing the importance of ephemeral streams to water quality regulation.
...
We note that our simple model uses only interpolated precipitation data, runoff data, and an operational definition for streamflow. It reasonably matches in situ sensor data of catchment-averaged ephemeral flow frequency (Fig. `r fig_nums("figNflw", display="n")`b-c). With that said, we stress that extremely little data exists on ephemeral flow frequency at the drainage network scale (see materials and methods). Future work should explore ephemeral runoff generation across distributed sensor networks to refine our modeling.
...
### Funding
CBB was supported by a NASA FINESST fellowship (award 80NSSC21K1591) and a NSF RoL FELS RAISE grant to CJG (principal investigator: PAR, award 1840243).
...
All data are available in the manuscript or the supplementary materials. Code used to run our analysis and build our figures is available at https://github.com/craigbrinkerhoff/CONUS_ephemeral.
