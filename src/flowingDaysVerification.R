...
#' Reynold's Creek, ID: https://doi.org/10.1029/2001WR000413 (29 years of data)
...
#' More Arizona data for various arid catchments https://doi.org/10.1016/j.jaridenv.2016.12.004 (2 years of data)
...
wrangleFlowingFieldData <- function(path_to_data){
  runoff_thresh <- 0 #threshold for 'streamflow generation' --> equals 1mm/hr for a full day
  ...
  reynoldsCreek$drainage_area_km2 <- ifelse(reynoldsCreek$site == 'summitWash', 83*0.01,
                                            ifelse(reynoldsCreek$site == 'flats', 0.9*0.01,
                                                   ifelse(reynoldsCreek$site == 'nancy_gulch',1.3*0.01, 13.4*0.01)))

  reynoldsCreek <- dplyr::group_by(reynoldsCreek, site, date) %>% #hourly to mean daily flow
    dplyr::summarise(watershed=first(watershed),
              site=first(site),
              runoff_mm = sum(((discharge*1000000000)/(drainage_area_km2*0.01*1000000000000))*60*24), #m3/s to mm in the day
  ...
  ontario <- data.frame('watershed'='geulph',
                           'site'='a',
                           'period_of_record_yrs'=mean(geulph$period_of_record_dys), #2008-2014
  ...
  output <- results_all %>%
    ...
    dplyr::summarise(n_flw_d = mean(ma_num_flowing_days),0) %>% #mean across watershed reaches
    dplyr::mutate(lat=c(36.020631, 43.54667, 32.711123, 31.540278, 33.33333333, 35.228431, 43.187, 37.47305556, 31.83341800, 31.66666667), #pulled from associated papers (general coords- Geulph long is moved ~15km east so it fits in model basin 0427)
           long=c(-78.982789, -80.059, -112.831066, -110.334113, -114.50000000, -106.840627, -116.774, -83.14333333, -110.85286400 , -110.00000000))
...
#' @import dplyr
#'
#' @return df of all wrangled field data sptially joined to HUC4 basin shapefiles
flowingValidate <- function(validationData, path_to_data, codes_huc02, combined_results){
  ...
  basins_overall <- dplyr::select(basins_overall, c('huc4', 'name', 'num_flowing_dys', 'geometry'))
...
#' @param runoffEffScalar_real: runoff scaler parameter used in model, held constant here
...
flowingValidateSensitivityWrapper <- function(flowingFieldData, runoffEffScalar_real, runoffMemory_real, runoff_threshs, path_to_data, combined_runoffEff) {
    huc4s = c('0302', '0427', '1507', '1505', '1503', '1302', '1705', '0510', '1505', '1505') #basins with field data

    out <- data.frame()
    num_flowing_dys <- rep(NA, length(huc4s))
    for(i in runoff_threshs){
      for(k in 1:length(huc4s)){
        num_flowing_dys[k] <- calcFlowingDays(path_to_data, huc4s[k], combined_runoffEff, i, runoffEffScalar_real, runoffMemory_real) #calculate ballpark number of flowing days
      }
