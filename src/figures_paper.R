...
mainFigureFunction <- function(shapefile_fin, net_0107_results, net_1009_results, net_1407_results, net_1305_results) {
  ...
  results$percQ_eph <- round(results$percQ_eph*100,0) #setup percent

  #labels for select basins with subplots
  results$ids_west <- ifelse(results$huc4 == '1009', 'B',
                         ifelse(results$huc4 == '1407', 'E', NA))
  results$ids_east <- ifelse(results$huc4 == '0107', 'D',
                          ifelse(results$huc4 == '1305', 'C',NA))
  ...
  results_map <- ggplot(results) +
    ...
    ggsflabel::geom_sf_label_repel(aes(label=ids_east),
                        ...
                        ylim=c(30,40),
                        xlim=c(-72.5,-70))+
    scale_fill_gradientn(name='% Ephemeral runoff',
                         colors=c('white', '#164d71', '#012a4a'),
  ...
  hydrography_0107 <- ggplot(net_0107, aes(color=perenniality, size=Q_cms)) +
    ...
    xlab('')+
    ylab('') +
    ggtitle(paste0('Merrimack River:\n', round(results[results$huc4 == '0107',]$totalephemeralQ_cms*86400*365*1e-9,0), ' km3/yr (', round(results[results$huc4 == '0107',]$percQ_eph,0), '%)'))
  ...
  hydrography_1407 <- ggplot(net_1407, aes(color=perenniality, size=Q_cms)) +
    ...
    xlab('')+
    ylab('') +
    ggtitle(paste0('Lower Green River:\n', round(results[results$huc4 == '1407',]$totalephemeralQ_cms*86400*365*1e-9,0), ' km3/yr (', round(results[results$huc4 == '1407',]$percQ_eph,0), '%)'))
  ...
  hydrography_1009 <- ggplot(net_1009, aes(color=perenniality, size=Q_cms)) +
    ...
    xlab('')+
    ylab('') +
    ggtitle(paste0('Powder/Tongue River:\n', round(results[results$huc4 == '1009',]$totalephemeralQ_cms*86400*365*1e-9,0), ' km3/yr (', round(results[results$huc4 == '1009',]$percQ_eph,0), '%)'))
...
##RIVER NETWORK MAP 1305-----------------------------------------------------------
  ...
  hydrography_1305 <- ggplot(net_1305, aes(color=perenniality, size=Q_cms)) +
    ...
    xlab('')+
    ylab('') +
    ggtitle(paste0('Rio Grande Endorheic:\n', round(results[results$huc4 == '1305',]$totalephemeralQ_cms*86400*365*1e-9,0), ' km3/yr (', round(results[results$huc4 == '1305',]$percQ_eph,0), '%)'))
    ...
    FFFF
    "
    comboPlot <- patchwork::wrap_plots(A=results_map, B=hydrography_1009, C=hydrography_1305, D=hydrography_0107+theme(legend.position='none'), E=hydrography_1407, F=hydrography_legend, design=design)
