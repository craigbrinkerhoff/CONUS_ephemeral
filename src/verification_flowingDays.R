...
wrangleFlowingFieldData <- function(path_to_data, ephemeralQDataset){
  ...
  output <- results_all %>%
    ...
    #also unfortunately need to be hardcoded because I had to find the non-gage sites manually in their papers or online....
...
#'
#' @return df of all wrangled field data spatially joined to HUC4 basin shapefiles
flowingValidate <- function(validationData, path_to_data, codes_huc02, combined_results, combined_runoffEff){ #combined_numFlowingDays_mc
