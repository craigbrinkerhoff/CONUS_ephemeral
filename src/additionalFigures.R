...
lengthMapFigureFunction <- function(shapefile_fin) {
  ...
  #results shapefile
  results <- dplyr::filter(results, is.na(percLength_eph)==0)
  results$percLength_eph <- results$percLength_eph * 100

  #HISTOGRAM INSET
  ephVolumeHist <- ggplot(results, aes(x=percLength_eph))+
  ...
  results_map <- ggplot(results) +
    ...
    geom_sf(aes(fill=percLength_eph),
            color='black',
            size=0.5) +
    geom_sf(data=states,
            color='black',
            size=1.25,
            alpha=0)+
    scale_fill_gradientn(name='% ephemeral network length',
                      colors = c("#335c67", "#99a88c", "#fff3b0", "#e09f3e", "#9e2a2b", "#540b0e"),
    ...
    ggsave('cache/lengthMap.jpg', results_map, width=20, height=15)
    return('see cache/lengthMap.jpg')
}
...
flowingMapFigureFunction <- function(shapefile_fin) {
  ...
  results_map <- ggplot(results) +
    draw_plot(ephVolumeHist,
              ...
              width = 25,
              height = 5.5)+ #histogram
    geom_sf(aes(fill=percQ_eph_flowing_scaled),
            color='black',
            size=0.5) +
    geom_sf(data=states,
            color='black',
            size=1.25,
            alpha=0)+
    scale_fill_gradientn(name='% water exported via U.S.\nephemeral streams when floqing',
                      colors = c("#335c67", "#99a88c", "#fff3b0", "#e09f3e", "#9e2a2b", "#540b0e"),
                      limits=c(0,100), #max/min values
                      guide = guide_legend(direction = "horizontal",
                                           title.position = "top"))+
...
#'
#' @param combined_numFlowingDays: aggreagted and tabulated model results across all HUC4 basins
#' @param combined_numFlowingDays_low: aggreagted and tabulated model results across all HUC4 basins using low runoff scenario
#' @param combined_numFlowingDays_high: aggreagted and tabulated model results across all HUC4 basins using high runoff scenario
#'
#' @import dplyr
#' @import tidyr
#' @import ggplot2
...
boxPlots_sensitivity <- function(combined_numFlowingDays, combined_numFlowingDays_low, combined_numFlowingDays_high){
  theme_set(theme_classic())

  #wrangle
  t <- c(t(combined_numFlowingDays)) #dplyr::select(combined_results, 'huc4', 'percQ_eph_flowing_scaled')
  numFlowingDays <- as.numeric(t[-length(t)])

  t <- c(t(combined_numFlowingDays_low)) #combined_results_low$percQ_eph_flowing_scaled
  znumFlowingDays_low <- as.numeric(t[-length(t)])

  t <- c(t(combined_numFlowingDays_high)) #combined_results_high$percQ_eph_flowing_scaled
  anumFlowingDays_high <- as.numeric(t[-length(t)])

  combined_results <- data.frame('numFlowingDays'=numFlowingDays, 'znumFlowingDays_low'=znumFlowingDays_low, 'anumFlowingDays_high'=anumFlowingDays_high)

  #discharge
  forPlot <- tidyr::gather(combined_results, key=key, value=value, c('numFlowingDays', 'znumFlowingDays_low', 'anumFlowingDays_high'))
  forPlot$key <- as.factor(forPlot$key)
  levels(forPlot$key) <- c('High runoff scenario', 'Model', 'Low runoff scenario')

  boxplotsSens <- ggplot(forPlot, aes(x=key, y=value, fill=key)) +
    geom_boxplot(color='black', size=1.25) +
    stat_summary(fun = mean, geom = "point", col = "darkred", size=8) +
    annotate('text', label=paste0('n = ', nrow(combined_results), ' basins'), x=as.factor('Model'), y=1.1, size=8)+
    scale_fill_brewer(palette='BrBG') +
    ylab('Average number days ephemeral streams flow per year') +
    xlab('')+
    theme(axis.text=element_text(size=20),
      axis.title=element_text(size=22,face="bold"),
      legend.text = element_text(size=17),
  ...
  ggsave('cache/boxPlots_sensitivity.jpg', boxplotsSens, width=9, height=10)
  return(boxplotsSens)
}
...
#' Build figure showing determination of runoff threshold
#'
...
runoffThreshCalibPlot <- function(calibResults){
  ...
  plot <- ggplot(calibResults, aes(x=thresh, y=mae)) +
    ...
    xlab('Stream Runoff Threshold [mm/dy]')+
    scale_x_log10()+
    theme(axis.text=element_text(size=20),
      axis.title=element_text(size=22,face="bold"),
      legend.text = element_text(size=17),
  ...
  ggsave('cache/runoffThresh_fitting.jpg', plot, width=9, height=8)
  return(plot)
}
